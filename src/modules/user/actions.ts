...
function loadRowData(citizenId: string, name: string) {
  return async (dispatch: any) => {
    ...
    dispatch(uiActions.setFlashMessage('กำลังอัพเดตข้อมูล...', 'info'))
    try {
      ...
      dispatch(
        uiActions.setFlashMessage(`อัพเดตข้อมูล ${name} สำเร็จ`, 'success')
      )
    } catch (err) {
      dispatch({ type: ROW_UPDATE_FAILURE })
      dispatch(
        uiActions.setFlashMessage(
          `อัพเดตข้อมูล ${name} ไม่สำเร็จ เกิดข้อผิดพลาด ${err?.response?.status}`,
          'error'
        )
      )
    }
